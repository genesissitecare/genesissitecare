...
add_theme_support( 'genesis-menus' , array(
	'primary'   => __( 'Header Menu', 'genesissitecare' ),
	'secondary' => __( 'After Header Menu', 'genesissitecare' ),
) );
...
add_theme_support( 'custom-header', array(
	...
	'uploads'            => true,
	'video'              => true,
	'wp-head-callback'   => 'gsc_custom_header',
) );
...
register_default_headers( array(
	'child' => array(
		...
		'description'   => __( 'Hero Image', 'genesissitecare' ),
	),
) );

// Register a narrow layout.
genesis_register_layout( 'narrow-content', array(
	'label' => __( 'Narrow Layout', 'genesissitecare' ),
	'img'   => get_stylesheet_directory_uri() . '/assets/images/narrow-layout.gif',
) );
...
// Remove Genesis Simple FAQ plugin styles.
add_filter( 'gs_faq_print_styles', '__return_false' );

add_action( 'wp_enqueue_scripts', 'gsc_scripts_styles', 99 );
/**
 * Enqueue theme scripts and styles.
 *
 * @return void
 */
function gsc_scripts_styles() {

	// Remove Simple Social Icons CSS (included with theme).
	wp_dequeue_style( 'simple-social-icons-font' );

	// Typekit fonts.
	wp_enqueue_style( 'typekit', 'https://use.typekit.net/cuj7tud.css', array(), CHILD_THEME_VERSION );
	...
	if ( gsc_is_woocommerce_page() ) {

		wp_enqueue_style( 'genesissitecare-woocommerce', get_stylesheet_directory_uri() . '/assets/styles/min/woocommerce.min.css', array(), CHILD_THEME_VERSION );

	}

	// Enqueue responsive menu script.
	wp_enqueue_script( 'genesissitecare', get_stylesheet_directory_uri() . '/assets/scripts/scripts.js', array( 'jquery' ), CHILD_THEME_VERSION, true );

	// Localize responsive menu script.
	wp_localize_script( 'genesissitecare', 'genesis_responsive_menu', array(
		'mainMenu'         => __( '', 'genesissitecare' ),
		'subMenu'          => __( '', 'genesissitecare' ),
		'menuIconClass'    => null,
